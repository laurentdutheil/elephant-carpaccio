...
func DefaultBacklog() Backlog {
	return Backlog{
		...
		NewUserStoryBuilder("EC-002").Description("Can fill parameters").RiskEstimation(20).IterationEstimation(1).Build(),
		NewUserStoryBuilder("EC-003").Description("Compute order value without tax").BusinessValueEstimation(NewDollar(Decimal(100000))).RiskEstimation(15).IterationEstimation(1).Build(),
		...
		NewUserStoryBuilder("EC-009").Description("Can handle float for 'number of items' AND 'price by item'").BusinessValueEstimation(NewDollar(Decimal(100000))).RiskEstimation(5).IterationEstimation(2).Build(),
		NewUserStoryBuilder("EC-010").Description("15% Discount").BusinessValueEstimation(NewDollar(Decimal(50000))).RiskEstimation(4).IterationEstimation(3).Build(),
		NewUserStoryBuilder("EC-011").Description("10% Discount").BusinessValueEstimation(NewDollar(Decimal(26000))).RiskEstimation(3).IterationEstimation(3).Build(),
